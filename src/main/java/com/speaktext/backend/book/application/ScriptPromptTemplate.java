...
public class ScriptPromptTemplate {
    ...
    public static final String TEMPLATE = """
            ...
            - 각 줄은 다음 형식을 따라야 합니다:
              - `scriptCharacter-id: "발화문"` ← 반드시 따옴표 안의 문장은 화자의 **발화문**입니다.
            ...
            - `"나"`라는 표현은 항상 **1인칭 화자**이며, 이미 지정된 `scriptCharacter-id`를 재사용해야 합니다.
            - 같은 인물로 판단되면 이전 등장인물의 `scriptCharacter-id`를 재사용하세요.
            - 임의로 `scriptCharacter-id`를 새로 부여하지 마세요. 반드시 기존 ID를 사용하세요.
            ...
            scriptCharacter-1: “그럼 혼자 하지 떼루 하디?”
            ```
            ...
            [스크립트 시작]
            scriptCharacter-1: "다녀왔어요."
            scriptCharacter-2: "어서 와, 점순아."
            나레이션 - narration: 점순은 신발을 벗고 조심스레 방으로 들어왔다.
            scriptCharacter-1: "오늘은 기분이 좀 이상해요."
            [스크립트 끝]

            [등장인물 업데이트 시작]
            {
            "점순 - scriptCharacter-1": "처음에는 '여성'으로 표현되었으나, 이름이 확인됨. 밝고 대담한 성격.",
            "엄마 - scriptCharacter-2": "점순의 어머니로 추정됨. 온화하고 다정한 말투."
            }
            [등장인물 업데이트 끝]
            ...
            - 이름이나 설명이 바뀌더라도 `scriptCharacter-id`는 **항상 동일**해야 합니다.
            - `scriptCharacter-id`가 바뀌거나 누락되는 일이 없도록 주의하세요.
