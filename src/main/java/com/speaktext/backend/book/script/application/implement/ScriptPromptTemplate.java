...
public class ScriptPromptTemplate {
    ...
    public static final String TEMPLATE = """
            ...
            - 각 줄은 다음 형식을 따라야 합니다:
              - `script-character-id: "발화문"` ← 반드시 따옴표 안의 문장은 화자의 **발화문**입니다.
            ...
            - `"나"`라는 표현은 항상 **1인칭 화자**이며, 이미 지정된 `script-character-id`를 재사용해야 합니다.
            - 같은 인물로 판단되면 이전 등장인물의 `script-character-id`를 재사용하세요.
            - 임의로 `script-character-id`를 새로 부여하지 마세요. 반드시 기존 ID를 사용해.
            ...
            script-character-1: “그럼 혼자 하지 떼루 하디?”
            ```
            > 🔎 이처럼 **따옴표가 있는 문장**은 scriptCharacter-1의 발화문으로, \s
            > **따옴표 없이 배경을 설명하는 문장**은 `나레이션 - narration`으로 처리해야 합니다.

            📌 **응답 형식 예시**:

            [스크립트 시작]
            script-character-1: "다녀왔어요."
            script-character-2: "어서 와, 점순아."
            나레이션 - narration: 점순은 신발을 벗고 조심스레 방으로 들어왔다.
            script-character-1: "오늘은 기분이 좀 이상해요."
            [스크립트 끝]

            [등장인물 업데이트 시작]
            {
            "점순 - script-character-1": "처음에는 '여성'으로 표현되었으나, 이름이 확인됨. 밝고 대담한 성격.",
            "엄마 - script-character-2": "점순의 어머니로 추정됨. 온화하고 다정한 말투."
            }
            [등장인물 업데이트 끝]
            ...
            - 등장인물 업데이트에는 이전 등장인물도 반드시 포함되어야 해.
            - 이름이나 설명이 바뀌더라도 스크립트의 `script-character-id`는 항상 동일해야 해.
            - `script-character-id`가 바뀌거나 누락되는 일이 절대로 없어야 해.
            - 모든 원문 문장은 반드시 빠짐없이 포함되어야 하며, **절대 요약하거나 창작하지 마.
            - 텍스트가 불명확해도 그대로 사용하고, 가능한 범위에서만 추론하세요.
            - `script-character-id`의 id는 반드시 정수 숫자여야 합니다. `script-character-unknown`과 같은 형식은 절대 사용하지 마.
